export function benachrichtigungOnboardingHtml({
  unternehmensname,
  ansprechpartner,
  ...
  function row(label: string, value: string | null, last = false): string {
    ...
    const val = value?.trim()
      ? `<td style="padding:10px 18px;font-family:'Inter',Helvetica,Arial,sans-serif;font-size:13px;color:#2a2a2a;${border}">${value}</td>`
  ...
  const person2 = [freigabeVerantwortlicher2, emailFreigabeVerantwortlicher2].filter(Boolean).join(" · ") || null;
  ...
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0">
    ...
    <tr><td align="center" class="ec" style="padding:20px 48px 0 48px;">
      <p style="margin:0;font-family:'EB Garamond',Georgia,serif;font-weight:500;font-size:30px;line-height:1.2;color:#111111;letter-spacing:-0.01em;text-align:center;">
        Onboarding-Formular<br/><em style="font-style:italic;font-weight:400;color:#c8863c;">ausgefüllt.</em>
      </p>
    </td></tr>
    ...
    <tr><td class="ec" style="padding:16px 56px 0 56px;">
      <p style="margin:0;font-family:'Inter',Helvetica,Arial,sans-serif;font-size:14px;line-height:1.7;color:#4b5468;text-align:center;">
        Ein neuer Kunde hat das Onboarding-Formular ausgefüllt.<br/>Alle Daten wurden automatisch im CRM angelegt.
      </p>
    </td></tr>
    ...
    <tr><td class="ec" style="padding:28px 48px 0 48px;">
      <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="border:1px solid #e6e3dd;border-radius:12px;overflow:hidden;">
        ...
        <tr><td colspan="2" style="padding:10px 18px 8px;background:#f8f7f5;border-bottom:1px solid #e6e3dd;">
          <p style="margin:0;font-family:'EB Garamond',Georgia,serif;font-style:italic;font-size:10px;letter-spacing:0.20em;text-transform:uppercase;color:#9b9690;">Unternehmen</p>
        </td></tr>
        ...
        ${row("Social Media", socialMediaKanaele)}

        ${sectionHeader("Ansprechpartner")}
        ${row("Name", ansprechpartner)}
        ${row("E-Mail", emailAnsprechpartner)}
        ${row("Telefon", telefonnummer)}

        ${sectionHeader("Freigabe-Verantwortliche")}
        ${row("Person 1", person1)}
        ${row("Person 2", person2, true)}
